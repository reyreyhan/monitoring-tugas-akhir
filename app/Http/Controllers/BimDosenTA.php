...
class BimDosenTA extends Controller
{
    public function index() {
      ...
      return view('dosen.dosenta',compact('nip1','nip2','nip3','nip4'));
    }

    public function detail($nrp) {
      $data = SetTaMahasiswa::with('dosTA')->where('nrp',$nrp)->get();
      $data2 = BimbinganTaMahasiswa::with('dos')->where('nrp',$nrp)->where('nip',session('sessionUser')[0]->nip)->where('id_kategori','3')->get();
      return  view('dosen.bimbinganmahasiswa',compact(['data','data2']));
    }
