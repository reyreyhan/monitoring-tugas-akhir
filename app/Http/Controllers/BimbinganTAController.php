...
class BimbinganTAController extends Controller
{
    public function index() {
      ...
      $data2 = BimbinganTaMahasiswa::with(['dos'])->where('nrp',session('sessionUser')[0]->nrp)->where('id_kategori','3')->get();

      $data3 = Persyaratan::where('id_kategori','3')->first();
      $data4 = BimbinganTaMahasiswa::with(['dos'])->where('nrp',session('sessionUser')[0]->nrp)->where('status','1')->where('id_kategori','3')->count();

      return view('bimbinganta',compact(['data','data2','data3','data4']));
    }
