...
@include('include.headerkoor')
  <div class="content-wrapper">
    <div class="container-fluid">
      <!-- Breadcrumbs-->
      <h2 class="mb-3">Bimbingan TA Mahasiswa</h2>
        <!-- Icon Cards-->
          ...
          <div class="col-md-12">
            ...
            <ul class="list-group list-group-flush">
              ...
              <tbody>
                ...
                <tr>
                  <td>{{ $u->jenis->jenis }}</td>
                  <td><a href="{{ url('upload/TA')}}/{{ $u->file }}">Sudah Upload</a></td>

                    @if($u->status == 0)
                    <td>
                      <form method="POST" action="{{url('koor/ta')}}/{{$u->id}}">
                        {{ csrf_field() }}
                        {{ method_field('PUT') }}
                        <button type="submit" class="btn btn-success btn-sm" name="button">Approve</button>
                      </form>
                      <br>
                      <form method="POST" action="{{url('ta/delete')}}/{{$u->nrp}}/{{$u->id_kategori}}/{{$u->id_jenis}}/{{$u->id}}">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
